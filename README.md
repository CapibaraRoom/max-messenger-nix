...
Неофициальная деривация мессенджера Макс для NixOS. Может работать нестабильно, т.к. предназаначена для личного применения, но если кому нужно - пользуйтесь.
...
Nix-конигурация (требует gnome-keyring):
```
{ pkgs, inputs, ... }:
...
Применение (возможно, потребуется перезагрузиться):
```
...
За основу взят deb-пакет. Получение метаданных (можно узнать актуальную версию): [download .gz](https://download.max.ru/linux/deb/dists/stable/main/binary-amd64/Packages.gz)
